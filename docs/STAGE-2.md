...
The tables created by this Stage are `my_users`, `my_roles`, `my_permissions`, `my_role_user`, `my_permission_role`, `my_user_preferences`, `my_module_settings`, and `my_module_states`, always prefixed through `$wpdb->prefix`. `wp_users.ID` is the canonical identity key: `my_users` is an extension/profile table keyed by `wp_user_id`, not a parallel identity table. `my_role_user` and `my_user_preferences` also use `wp_user_id`; WordPress remains authoritative for login identity and capabilities. No `wp_` prefix is hard-coded in the migration path.
...
The Stage 2 migration is additive and versioned. It does not drop tables, reset a database, delete data or perform destructive alteration. `SchemaManager` owns ordering, migration locking, idempotent `dbDelta()` execution, version advancement, failure recovery and the non-destructive rollback plan. `dbDelta()` is only the low-level table helper. Activation calls the schema manager, while the diagnostic CLI exposes `wp mechanicyab migrate`. The migration requires a real WordPress database runtime; if no `$wpdb` is available, it fails explicitly rather than creating a mock.
