...
{
    /** @param \Closure(int): array<string,mixed>|null $mechanicResolver */
    ...
    private function assertOwner(int $mechanicId,int $actorId): void { $mechanic = ($this->mechanicResolver)($mechanicId); if(!is_array($mechanic) || (int)($mechanic['owner_user_id'] ?? 0) !== $actorId){throw new \DomainException('Mechanic ownership is required.');} }
}
