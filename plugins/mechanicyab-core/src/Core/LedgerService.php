...
final class LedgerService
{
    ...
    public function post(array $entries): bool
    {
        ...
        foreach ($entries as $entry) { $amount=(int)($entry['amount']??0); if ($amount <= 0 || !in_array($entry['direction']??'', ['debit','credit'], true) || ($currency !== null && $currency !== ($entry['currency']??'IRR'))) { throw new \InvalidArgumentException('Invalid or unbalanced ledger entry.'); } $currency ??= $entry['currency']??'IRR'; if (($entry['direction']??'')==='debit') {$debit += $amount;} else {$credit += $amount;} }
