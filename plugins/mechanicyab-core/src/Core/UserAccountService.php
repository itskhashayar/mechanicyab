...
final class UserAccountService
{
    ...
    public function createServiceRecord(int $userId, array $data): int { $this->assertUser($userId); if ((int) ($data['vehicle_id'] ?? 0) < 1 || empty($data['service_date'])) { throw new \InvalidArgumentException('Vehicle and service date are required.'); } return $this->repository->createServiceRecord($userId, $data); }
    public function serviceHistory(int $userId, int $vehicleId): array { $this->assertUser($userId); if ($vehicleId < 1) { throw new \InvalidArgumentException('Vehicle is required.'); } return $this->repository->serviceHistory($userId, $vehicleId); }
    ...
    public function notifications(int $userId): array { $this->assertUser($userId); return $this->repository->notifications($userId); }
    public function markNotificationRead(int $userId, int $notificationId): bool { $this->assertUser($userId); if ($notificationId < 1) { throw new \InvalidArgumentException('Notification is required.'); } return $this->repository->markNotificationRead($userId, $notificationId); }
